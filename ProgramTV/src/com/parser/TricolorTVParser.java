...
public class TricolorTVParser {
  ...
  public static List<TVItem> ParseData(){
	...
	try {
		json = readJsonFromUrl("https://www.tricolor.tv/ajax/program/grid.php?channel=select&channel-type=20&channel-name=0&channel-format=0&date_for_now=&day=&type=grid&favoriteChannels=undefined");
    ...
    while(keys.hasNext()){
    	...
    	//Add data to item
    	item.setNetwork(program.getString("channelName"));
       	item.setName(program.getString("title").replaceAll("&quot;", "'"));
    	item.setAirDate(airdate);
